...
def countcls():
    ...
    if image_counts <= 3:
        ...
        long_format = grouped_data.reset_index().melt(id_vars='class3', var_name='image3', value_name='count')
        # 使用Plotly绘制柱状图
        fig = px.bar(long_format, x='class3', y='count', color='image3', barmode='group')
        ...
        fig.update_layout(title='统计类别', xaxis_title='类别', yaxis_title='数量')
        str=["图像1:4<br>图像2:5<br>图像3:4<br>图像4:3","图像2:2","图像2:1","图像4:1"]
        fig.update_traces(hovertemplate='类别: %{x}<br>%{customdata}',
                          customdata=str)
